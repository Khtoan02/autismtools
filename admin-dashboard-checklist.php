...
<head>
    ...
    <title>Checklist Analytics Dashboard - DawnBridge</title>

    <!-- 2. NẠP THƯ VIỆN CẦN THIẾT (CDN) -->
...
<script type="text/babel">
    ...
    // Lấy các component từ thư viện Recharts Global
    const { 
        BarChart, Bar, XAxis, YAxis, CartesianGrid, Tooltip, Legend, ResponsiveContainer, 
        PieChart, Pie, Cell, AreaChart, Area, LineChart, Line, ComposedChart, Scatter, 
        Radar, RadarChart, PolarGrid, PolarAngleAxis, PolarRadiusAxis 
    } = window.Recharts;

    // --- CẤU HÌNH ICON (Mapping Lucide sang React Component) ---
    // Vì không dùng npm, ta tạo Wrapper Component để gọi Lucide Global
    const IconWrapper = ({ name, size = 18, className = "" }) => {
        useEffect(() => {
            if (window.lucide) window.lucide.createIcons();
        });
        // Lucide render SVG vào thẻ <i> hoặc <span> có thuộc tính data-lucide
        return <i data-lucide={name} className={className} style={{ width: size, height: size, display: 'inline-flex' }}></i>;
    };

    // Định nghĩa lại các Icon Component được dùng trong code gốc
    const LayoutDashboard = (p) => <IconWrapper name="layout-dashboard" {...p} />;
    const Activity = (p) => <IconWrapper name="activity" {...p} />;
    const ClipboardCheck = (p) => <IconWrapper name="clipboard-check" {...p} />;
    const UserCheck = (p) => <IconWrapper name="user-check" {...p} />;
    ...
    const MapPin = (p) => <IconWrapper name="map-pin" {...p} />;
    const Baby = (p) => <IconWrapper name="baby" {...p} />;
    const TrendingUp = (p) => <IconWrapper name="trending-up" {...p} />;
    const LinkIcon = (p) => <IconWrapper name="link" {...p} />;

    // --- DỮ LIỆU MẪU (MOCK DATA - CẦN THAY THẾ BẰNG API THẬT SAU NÀY) ---
    ...
    const detailedFunnelData = [
      ...
      { step: 'Lưu Kết quả (SĐT)', users: 620, dropoff: 65, time: '3m15s' },
    ];

    const sectionPerformanceData = [
      { name: 'G1. Tiêu hoá & Đi ngoài', avgTime: 35, quitRate: 5, yesRate: 72, status: 'Quan trọng' },
      { name: 'G2. Hấp thu & Ăn uống', avgTime: 45, quitRate: 12, yesRate: 55, status: 'Cảnh báo' }, 
    ...
    const contentEngagementData = [
      { name: 'Bài: Chế độ GFCF (Mức Nặng)', views: 600, clicks: 180 },
    ...
    const UpdatedDashboard = () => {
      ...
      return (
        <div className="min-h-screen bg-slate-50 font-sans text-slate-800 pb-12 relative p-4 sm:p-6">
          ...
          {/* Top Nav */}
          <nav className="bg-white border-b border-gray-200 rounded-2xl sticky top-4 z-30 px-6 py-4 shadow-sm mb-6">
            <div className="flex flex-col md:flex-row md:justify-between md:items-center gap-4">
              <div className="flex items-center gap-3">
                <div className="w-10 h-10 bg-teal-600 rounded-xl flex items-center justify-center text-white font-bold shadow-lg shadow-teal-200">
                  DB
                </div>
                <div>
                  <h1 className="text-xl font-bold text-gray-800 leading-tight">Analytics: Checklist Tiêu Hóa</h1>
                  <p className="text-xs text-gray-500 font-medium">Phiên bản chính thức 2024</p>
                </div>
              </div>
              
              <div className="flex items-center gap-3 self-end md:self-auto">
                <div className="flex bg-gray-100 p-1 rounded-lg mr-2">
                  <button onClick={() => setActiveTab('overview')} className={`px-4 py-2 text-xs font-bold rounded-md transition-all ${activeTab === 'overview' ? 'bg-white text-teal-700 shadow-sm' : 'text-gray-500 hover:text-gray-700'}`}>Tổng quan</button>
                  <button onClick={() => setActiveTab('behavior')} className={`px-4 py-2 text-xs font-bold rounded-md transition-all ${activeTab === 'behavior' ? 'bg-white text-teal-700 shadow-sm' : 'text-gray-500 hover:text-gray-700'}`}>Hành vi & UX</button>
                  <button onClick={() => setActiveTab('medical')} className={`px-4 py-2 text-xs font-bold rounded-md transition-all ${activeTab === 'medical' ? 'bg-white text-teal-700 shadow-sm' : 'text-gray-500 hover:text-gray-700'}`}>Y Khoa</button>
                </div>

                <select 
                  value={timeRange} 
                  onChange={(e) => setTimeRange(e.target.value)}
                  className="bg-white border border-gray-200 text-gray-700 text-xs font-medium rounded-lg focus:ring-teal-500 focus:border-teal-500 block px-3 py-2 shadow-sm"
                >
                  <option value="today">Hôm nay</option>
                  <option value="7days">7 ngày qua</option>
                  <option value="30days">30 ngày qua</option>
                </select>
                <button onClick={refreshData} className="p-2 text-gray-400 hover:text-teal-600 transition-colors bg-white rounded-full hover:bg-gray-100 border border-gray-200 shadow-sm">
